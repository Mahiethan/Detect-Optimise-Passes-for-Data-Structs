...
//iterate through main function
  ...
  //get next instruction
    ...
    //get first operand - a ptr- this is where the array is stored - save this % value in a vector
...
struct detectAoS : public PassInfoMixin<detectAoS> {
    PreservedAnalyses run(Module &M, ModuleAnalysisManager &AM) {
        ...
        vector<tuple<string, int, Value*>> functionInspectionList; //vector that stores functions that need to be inspected for GPE instructions in order to fully determine that a dynamic AoS exists 
        ...
        { 
         ...
         for (auto &F : M) //iteration of functions to determine if any remaining arrays in vector are dynamic AoS
         {
            ...
            //check if current function is in inspection list before starting iterating through its instructions
            for(auto it = functionInspectionList.begin(); it !=functionInspectionList.end(); it++)
            {
              if(get<0>(*it) == funcName)
              {
                 argIndex = get<1>(*it);
                 inspect = true;
                 possible_AoS = get<2>(*it);
                 functionInspectionList.erase(it);
                 break;
              }
            }
