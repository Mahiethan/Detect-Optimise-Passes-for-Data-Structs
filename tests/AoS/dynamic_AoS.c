...
void populate()
{
    populateNodeOne(globalThree,100); //dynamic AoS 4 - DETECTED!
    int f = 100;
...
void populateParam2(struct nodeOne* array, int n)
{
    ...
    innerArrayOne[0].a = 99; //dynamic AoS 6 - DETECTED
...
void populateParam4(struct nodeOne* array, int n)
{
    ...
    struct nodeTwo* arrayTwo = (struct nodeTwo*) calloc(100,sizeof(struct nodeTwo)); //dynamic AoS with duplicate name - need to differentiate somehow - maybe write as (arrayname,functionname) pair in 'confirmed' list
    arrayTwo[23].a = 1.02f; //dynamic AoS 8 - DETECTED!
...
int main()
{
    ...
    int* intArray = (int*) calloc(c,sizeof(int)); //not an AoS
    intArray[55] = 23525;
    ...
    populateNodeTwo(globalTwo,d); //dynamic AoS 3 - DETECTED!
    ...
    populate(); //adds two dynamic AoS: globalThree and globalFour

    populateParam0(arrayThree,e); //adds two dynamic AoS: innerArrayOne and arrayThree

    //dynamic AoS 9 - DETECTED
    // globalFive[1].a = 100;
    // printf("%d\n",globalFive[1].a);
