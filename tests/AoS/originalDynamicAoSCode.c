...
void populateTwo(struct nodeThree* arrayFive, int n)
{
    populateNodeThree(n, arrayFive);
     populateNodeThree(n, globalFive);
    // globalFive = (struct nodeThree*) malloc(100*sizeof(struct nodeThree)); //Redefining globalFive - should not count it again
    // populateNodeThree(n, globalFive);
    // globalSix = (struct nodeThree*) malloc(100*sizeof(struct nodeThree)); 
...
void populate(struct nodeThree* arrayFive, int n)
{
    // populateNodeThree(n, arrayFive);
    //     populateNodeThree(n, globalFive);
    populateTwo(arrayFive,n);
    printf("%lu\n",sizeof(globalSix));
    // globalSix[0].a = 2; //should detect this
...
void populateArrayFive(int a, int n, struct nodeThree* arrayFive, int c, int d)
{
...
int main()
{
    ...
    (void) AoS_Start;
    struct nodeOne* arrayOne = (struct nodeOne*) malloc(n*sizeof(struct nodeOne)); //dynamic AoS (stored on heap) (of size n) //AoS 1
    (void) AoS_End;
    ...
    populateArrayFive(1,n,arrayFive,3,4);
