...
opt -load-pass-plugin=../../../../passes/optimiseAoS/peelAoS/peelAoS.so -passes="peelAoS" -time-passes < unpeeledStruct.ll > peeledStruct.bc 2> output.txt
...
opt -load-pass-plugin=../../../../passes/optimiseAoS/peelAoS/peelAoS.so -passes="peelAoS" -time-passes < unpeeledStructTwo.ll > peeledStructTwo.bc 2> output.txt
file_content=$(sed -n "4p" "output.txt") #originally "4p" without any errs() in optimisation pass
echo $file_content
rm output.txt
