...
llc orderedTwo.bc -o orderedTwo.s > /dev/null 2>&1 #compiling to native assembly using the LLC code generator
gcc orderedTwo.s -o orderedTwo.native > /dev/null 2>&1 #Assemble the native assembly language file into a program
...
/usr/bin/time -v ./orderedTwo.native >> output.txt 2>&1
value_a_ordered=$(sed -n "2p" "output.txt") 
echo $value_a_ordered
value_b_ordered=$(sed -n "4p" "output.txt") 
echo $value_b_ordered
